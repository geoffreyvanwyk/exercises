(ns caves-of-clojure.core
  (:use [caves-of-clojure.world :only [random-world]])
...
(defmethod draw-ui :lose [ui game screen]
  ...
  (s/put-string screen 0 11 "Press escape to exit, anything else to go."))
...
(defmethod process-input :start [game input]
  (-> game
      ...
      (assoc :uis [(map->UI {:kind :play})])))

(defmethod process-input :play [game input]
  (case input
    :enter     (assoc game :uis [(map->UI {:kind :win})])
    :backspace (assoc game :uis [(map->UI {:kind :lose})])
    game))

(defmethod process-input :win [game input]
  (if (= input :escape)
    (assoc game :uis [])
    (assoc game :uis [(map->UI {:kind :start})])))

(defmethod process-input :lose [game input]
  (if (= input :escape)
    (assoc game :uis [])
    (assoc :uis [(map->UI {:kind :start})])))
